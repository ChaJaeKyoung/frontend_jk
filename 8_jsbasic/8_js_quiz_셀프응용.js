...
// 위 HTML 요소의 내용(Content)을 콘솔 출력하시오!
  ...
  const boxCt = document.querySelector('.box');
  console.log(boxCt.textContent);

// Q18
// 위 코드(Q17)에서 선택한 요소에 클릭(Click) 이벤트를 추가해,
// 클릭 시 'Hello~'를 콘솔 출력하시오!

  boxCt.addEventListener('click',function() {
    console.log('Hello~');
  });
...
// 위 2개의 요소에 JS로 class="hello"를 추가하시오!
  const addCls = document.querySelectorAll('span');
  console.log(addCls);
  addCls.forEach(function(dd,index){
    dd.classList.add('hello');
    if (index==1) {
      dd.classList.add('second');
      dd.classList.remove('hello');
    }
  })
...
// 'active'가 포함되어 있으면, '포함됨!'을 콘솔 출력하시오!
  ...
  if (boxCt.classList.contains('active')) {
    console.log('포함됨!');
  }
