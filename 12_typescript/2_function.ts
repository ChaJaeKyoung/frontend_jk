...
function multifly(x: number):number { //리턴되는 값에 타입지정도 하고싶다
  return x * 2 ;  
}
multifly(10);
const multifly2 = (x:number):number => x * 3 ;
...
const printName = (name?: string): void => name? console.log(`안녕하세요 ${name}님!`):console.log(`이름이 없습니다.`);
;
printName('재경');
printName();
...
const printChiper = (c: string): number => c.length;
